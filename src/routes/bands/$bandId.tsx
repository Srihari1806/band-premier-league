    ...
    <PageShell>
      ...
      <div className="bg-background border-b border-border/40 py-3">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <Link
            to="/bands"
            className="text-xs text-muted-foreground hover:text-primary-glow flex items-center gap-1 transition"
          >
      ...
      <section className="relative">
        <div className="h-64 md:h-80 w-full overflow-hidden bg-slate-900">
          <img
            src={band.banner_image}
            alt={`${band.band_name} Banner`}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5">
            ...
            <div className="space-y-1 text-left">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-1">
                <span className="text-primary-glow font-bold uppercase tracking-wider">
                  {band.genre}
                </span>
                <span className="flex items-center gap-1">
                  <MapPin size={12} /> {band.home_city}
                </span>
                {band.formed_year && (
                  <span className="flex items-center gap-1">
                    <Calendar size={12} /> Formed {band.formed_year}
                  </span>
                )}
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Panel title="About / Story">
            <div className="space-y-4 text-sm text-muted-foreground leading-relaxed">
              ...
              {band.mission && (
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-primary-glow font-bold mt-4">
                    Mission
                  </h4>
          ...
          {(band.stage_rider || band.technical_requirements) && (
            <Panel title="Technical Specs">
              <div className="space-y-4 text-xs leading-relaxed text-muted-foreground">
                {band.stage_rider && (
                  <div>
                    <h4 className="text-xs uppercase tracking-wider text-primary-glow font-bold mb-1">
                      Stage Rider Summary
                    </h4>
                    <p className="bg-secondary/40 border border-border p-3 rounded text-white/90">
                      {band.stage_rider}
                    </p>
                  </div>
                )}
            ...
            <div className="space-y-3 pt-1 text-sm">
              ...
              {band.languages && (
                <div className="flex justify-between border-b border-border/50 pb-2">
                  ...
                  <span className="text-white font-semibold text-xs truncate max-w-[150px]">
                    {band.languages}
                  </span>
                </div>
              )}
          ...
          <Panel title="Schedules & Bookings">
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground leading-relaxed">
                Connect with the BPL Management team to book this artist for college festivals,
                lounge gigs, and brand activations.
              </p>
              <a
              ...
              <div className="space-y-2 pt-1">
                {band.saavn_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex justify-between items-center text-xs text-muted-foreground hover:text-primary-glow transition border-b border-border/40 pb-2"
                  >
                ...
                {band.website_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex justify-between items-center text-xs text-muted-foreground hover:text-primary-glow transition pb-1"
                  >
